...
class Plotter():
    ...
    def timeplot(self, path_figs, model_v, Tstop):
        ...
        if model_v != 'M0':
            ...
            for n in range(Tstop+1):
                ...
                # get data
                Na_i = self.read_from_file(n, 0)
                Na_e = self.read_from_file(n, 1)
                K_i = self.read_from_file(n, 2)
                K_e = self.read_from_file(n, 3)
                Cl_i = self.read_from_file(n, 4)
                Cl_e = self.read_from_file(n, 5)
                phi_i = self.read_from_file(n, 6)
                phi_e = self.read_from_file(n, 7)
    ...
    def spaceplot(self, path_figs, model_v, n):
        ...
        if model_v != 'M0':
            ...
            K_i_ = self.read_from_file(n, 2)
            K_e_ = self.read_from_file(n, 3)
            Cl_i_ = self.read_from_file(n, 4)
            Cl_e_ = self.read_from_file(n, 5)
            phi_i_ = self.read_from_file(n, 6)
            phi_e_ = self.read_from_file(n, 7)
