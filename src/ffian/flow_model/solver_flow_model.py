...
class Solver():
    ...
    def PDE_solver(self):
        ...
        alpha_i_init = float(self.model.alpha_i_init) # initial volume fraction - ICS
        ...
        phi_i_ = ww_[-4]           # ICS potential from previous time step
        phi_e = ww[-3]             # ECS potential 
        v_phi_e = vv[-3]           # test function for ECS potential 
        phi_e_ = ww_[-3]           # ECS potential from previous time step
        ...
        p_m_init = params['p_m_init'] # intitial hydrostatic pressure difference across the membrane
        tau = K_m*(alpha_i - alpha_i_init)
        p_i = p_e + tau + p_m_init
        
        # intra- and extracellular fluid velocities
        if self.model.model_v == 'M1':
            u_i = - kappa[0]*(grad(p_i)) 
            u_e = - kappa[1]*grad(p_e) 
        elif self.model.model_v == 'M2':    
            u_i = - kappa[0]*(grad(p_i) - R*temperature*grad(a_i/alpha_i)) 
            u_e = - kappa[1]*grad(p_e) 
        elif self.model.model_v == 'M3':    
            u_i = - kappa[0]*(grad(p_i) - R*temperature*grad(a_i/alpha_i)) 
            u_e = - kappa[1]*grad(p_e) - eps_r*eps_zero*zeta*grad(phi_e)/mu
        else:
        ...
        # add contribution from hydrostatic pressure to transmembrane water flux
        w_m += p_i - p_e
        ...
        for n in range(N_ions):
            ...
            j_i = - D_i*(grad(k_i) + z[n]*F*k_i/(R*temperature)*grad(phi_i)) + u_i*k_i

            # ECS ion flux for ion n - (mol/m^2*s)
            j_e = - D_e*(grad(k_e) + z[n]*F*k_e/(R*temperature)*grad(phi_e)) + u_e*k_e

            # form for conservation of ion n in ICS
            A_k_i += 1.0/(self.dt*gamma_m)*inner(alpha_i*k_i - alpha_i_*k_i_, v_k_i)*dx \
                   - 1.0/gamma_m*inner(alpha_i*j_i, grad(v_k_i))*dx \
                   + inner(j_m[n], v_k_i)*dx
            
            # form for conservation of ion n in ECS 
            A_k_e += 1.0/(self.dt*gamma_m)*inner(alpha_e*k_e - alpha_e_*k_e_, v_k_e)*dx \
                     - 1.0/gamma_m*inner(alpha_e*j_e, grad(v_k_e))*dx \
                     - inner(j_m[n], v_k_e)*dx \
                     - inner(j_in[n], v_k_e)*dx
        ...
        A_alpha_i += 1.0/self.dt*inner(alpha_i - alpha_i_, v_alpha_i)*dx \
                       ...
                       + gamma_m*inner(w_m,v_alpha_i)*dx
        ...
        bcs = [DirichletBC(self.W.sub(self.N_unknowns-2), Constant(0.0), point)]
        ...
        prm['newton_solver']['relative_tolerance'] = 1E-4             # set relative tolerance
