...
class ModelBase():
    ...
    def set_parameters(self):
        ...
        z_0 = Constant(0.0)            # valence - immobile ions (a) - will be calculated at the end of this function 
        z = [z_Na, z_K, z_Cl, z_0]
        ...
        eta_m = Constant(8.14e-14)     # membrane water permeability - (m/(Pa*s))

        # compartmental fluid flow parameters
        kappa_i = Constant(1.8375e-14) # ICS water permeability - (m^4/(N*s))
        kappa_e = Constant(1.8375e-14) # ECS water permeability - (m^4/(N*s))
        kappa = [kappa_i, kappa_e]
        eps_r = Constant(84.6)         # relative permittivity
        eps_zero = Constant(8.85e-12)  # vacuum permittivity - (F/m)
        zeta = Constant(-22.8e-3)      # zeta-potential - (V)
        mu = Constant(6.4e-4)          # water viscosity - (Pa*s)
    ...
    def set_initial_conditions(self, options:dict=None):
        ...
        self.alpha_i_init = in_options['alpha_i'] # volume fraction - glia
        self.alpha_e_init = in_options['alpha_e'] # volume fraction - ECS
        ...
        self.p_e_init = '0.0'                # extracellular hydrostatic pressure
        self.c_init = '0.0'                  # Lagrange multiplier

        self.inits_PDE = Constant((self.alpha_i_init, \
                                     self.Na_i_init, \
                                     self.Na_e_init, \
                                     self.K_i_init, \
                                     self.K_e_init, \
    ...
    def j_pump(self, K_e, Na_i):
        ...
        # define and return flux (mol/(m^2s))
        j = rho_pump*((Na_i**1.5 / (Na_i**1.5 + P_Nai**1.5)) * (K_e / (K_e + P_Ke)))
